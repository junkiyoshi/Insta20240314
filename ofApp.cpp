...
void ofApp::update() {
	...
	int radius = 5;
	while (this->log_list.size() < 25000) {
		...
		this->color_list.push_back(ofColor(ofRandom(0, 39)));
	...
	for (int i = this->log_list.size() - 1; i >= 0; i--) {
		...
		auto deg = ofMap(ofNoise(glm::vec4(this->log_list[i].back() * 0.01, ofGetFrameNum() * 0.00003)), 0, 1, -360, 360);
		auto theta = ofMap(ofNoise(glm::vec4(this->log_list[i].back() * 0.01, (ofGetFrameNum() + 10000) * 0.00003)), 0, 1, -360, 360);
